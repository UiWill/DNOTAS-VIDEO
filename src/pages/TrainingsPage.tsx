...
const TrainingsPage = () => {
  ...
  const fetchData = async () => {
    try {
      const [modulesRes, lessonsRes, progressRes] = await Promise.all([
        ...
        supabase.from("training_lessons").select("id, module_id, title, description, video_url, cover_url, display_order").eq("is_active", true).order("display_order"),
        user
          ? supabase.from("training_progress").select("lesson_id, completed, watch_position_seconds").eq("user_id", user.id)
  ...
  const selectLesson = (lesson: TrainingLesson) => {
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h1 className="mt-1 text-xl font-bold tracking-tight sm:text-2xl">Treinamentos</h1>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {modules.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-lg font-medium text-muted-foreground">Nenhum treinamento disponível</p>
                <p className="text-sm text-muted-foreground/70 mt-1">Os treinamentos serão disponibilizados em breve.</p>
              </div>
            ) : (
